...
def angle_cv(path):
    ...
    dots = np.array([[(elem[0]+elem[2])/2,(elem[1]+elem[3])/2] for elem in high_bars])
    x_nums = np.array([(elem[0]+elem[2])/2 for elem in high_bars])
    y_nums = np.array([(elem[1]+elem[3])/2 for elem in high_bars])
    a = (len(x_nums)*np.sum(x_nums*y_nums) - np.sum(x_nums)*np.sum(y_nums))/(len(x_nums)*np.sum(x_nums*x_nums) - np.sum(x_nums)**2)
    return a